// Package config provides configuration management utilities for the max-ui panel,
...
// GetVersion returns the version string of the max-ui application.
func GetVersion() string {
	return strings.TrimSpace(version)
}

// GetName returns the name of the max-ui application.
func GetName() string {
...
// /etc/max-ui. Legacy installs are migrated from LegacyDBPath on first init.
...
// rather than derived from GetName() so the on-disk DB is always "max-ui.db".
const dbBaseName = "max-ui"
...
//   - <bindir>/x-ui.db — the prior next-to-binary name (before the max-ui rename)
//
// It deliberately does NOT reach into /etc/max-ui — a DB left there is not adopted.
