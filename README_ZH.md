...
- ? **Shadowsocks** ????? **AES-256-GCM** ? **AES-128-GCM** ?? **Encryption**
- ? **Inbound** ? **Outbound** ??? **XHTTP Object**
- **[WARP-CLI](https://github.com/Sir-MmD/warp-cli)**(Cloudflare ????)??????
- ??[????? **Xray-core**](https://github.com/Sir-MmD/Xray-core) ??,???? **Shadowsocks** ?????Unsupported Cipher???
...
WireGuard (C)?AmneziaWG ?? IKEv2 ? **PSK** / **EAP-TLS** ?????????????,????? RADIUS ??????;?????,????????????????,??? **User Limit** ???**RBridge**(Radius Bridge)?????????:??????????,?? **Sweeper** ???(poll)?????????,????(quota)????????? **User Limit** K(?????? evict ??),?????????? RADIUS ???????????? **RADIUS** ?????? **nftables** ?????????,??????????????????????????,?????? Xray **dokodemo-door** ???????
