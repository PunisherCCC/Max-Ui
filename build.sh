#!/usr/bin/env bash
#
# build.sh — build the complete, self-contained max-ui binary. Run it, that's it.
#
#   ./build.sh
#
...
# fetches the latest geo files — then compiles build/out/max-ui-<arch> with everything
...
hr
step "max-ui build ${_CD:-}(${ARCH})${_CR:-}"
hr
...
OUT_BIN="$OUT_DIR/max-ui-$ARCH"
step "compiling max-ui"
mkdir -p "$OUT_DIR"
CGO_ENABLED=1 go build -o "$OUT_BIN" main.go

hr
ok "done: ${_CB:-}$(ls -lh "$OUT_BIN" | awk '{print $5}')${_CR:-} -> ${_CB:-}build/out/max-ui-${ARCH}${_CR:-}"
info "run it:  ./build/out/max-ui-${ARCH}"
hr
