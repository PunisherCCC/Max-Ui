...
#
#   PREFIX_IN_TREE  = /usr/libexec/max-ui/libreswan   (== LibreswanBundleRoot)
...
PREFIX=/usr/libexec/max-ui/libreswan     # must equal backend.LibreswanBundleRoot
...
#!/bin/sh
# max-ui bundled libreswan 'ipsec' launcher — generated by libreswan-bundle.sh.
B=$PREFIX
...
#!/bin/sh
# max-ui bundled pluto launcher — generated by libreswan-bundle.sh.
B=$PREFIX
export IPSEC_EXECDIR="\$B/libexec/ipsec"
export IPSEC_LIBEXECDIR="\$B/libexec/ipsec"
export LD_LIBRARY_PATH="\$B/lib:\${LD_LIBRARY_PATH:-}"
...
# /usr/libexec/max-ui/libreswan exactly.
mkdir -p /out
