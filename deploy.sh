...
DEST="$DEST_DIR/$ASSET"
UNIT="max-ui"
# The management menu (`max-ui`). Installed from INSIDE the binary we just placed
...
# in max-ui.sh, which is sourced further below once the menu script is installed.
# It used to be defined here and copied into the menu, which is exactly how two
# acme.sh flows drift apart. Sourcing (rather than running `max-ui ssl`) keeps it
...
# Also reap a panel launched OUTSIDE systemd (a bare ./max-ui): the stop above only
...
if command -v pkill >/dev/null 2>&1; then
    pkill -x max-ui 2>/dev/null || true
...
if [[ "$MODE" == "update" && -f "$DB" ]]; then
    ...
    ts="$(date +%Y%m%d-%H%M%S)"
    backup="$BACKUP_DIR/max-ui_${OLD_VER:-unknown}_${ts}.db"
...
# the compiled-in /opt/max-ui default.
export VPNUI_BIN="$DEST"
if "$DEST" install-menu >/dev/null 2>&1 && [[ -r "$MENU" ]]; then
    ok "management menu -> ${MENU}  (run: ${TEAL}max-ui${R})"
    # Bring in obtain_letsencrypt_cert: the single implementation, shared rather
    # than copied. max-ui.sh does nothing at top level when sourced (its menu is
    # behind a sourced/executed guard), so this only defines functions.
    # shellcheck source=max-ui.sh
    source "$MENU"
else
    warn "could not install ${MENU}, so the 'max-ui' menu is unavailable on this host."
...
if [[ "$MODE" == "install" ]]; then
    # Optional migration: import an existing 3x-ui (or max-ui) backup database before
...
if [[ -x "$MENU" ]]; then
    act "manage:  ${TEAL}max-ui${R}  (update, login, start/stop, Xray, SSL)"
fi
