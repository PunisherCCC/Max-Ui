// Package logger provides logging functionality for the max-ui panel with
...
const (
	...
	logFileName      = "max-ui.log"            // Log file name
...
func InitLogger(level logging.Level) {
	newLogger := logging.MustGetLogger("max-ui")
	...
	if consoleBackend := initDefaultBackend(); consoleBackend != nil {
		...
		leveledBackend.SetLevel(level, "max-ui")
	...
	if fileBackend := initFileBackend(); fileBackend != nil {
		...
		leveledBackend.SetLevel(logging.DEBUG, "max-ui")
		backends = append(backends, leveledBackend)
	}
