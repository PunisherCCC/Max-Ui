#!/usr/bin/env bash
# max-ui test unit — root entrypoint.
...
for arg in "$@"; do
  case "$arg" in
    -h|--help)
      cat <<'EOF'
max-ui test unit — parallel incus E2E matrix over server distros.
...
if [[ ! -f "$SCRIPT_DIR/test_subject/max-ui" ]]; then
  echo "FATAL: '$SCRIPT_DIR/test_subject/max-ui' not found. Place the prebuilt max-ui binary (and its bin/ dir) in test_subject/." >&2
  exit 1
fi
