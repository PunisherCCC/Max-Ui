...
# ---------------------------------------------------------------------------
FEAT_PATHS = {
    "pppd": "/usr/libexec/max-ui/sbin/pppd",
    "pptpctrl": "/usr/libexec/max-ui/pptpctrl",
    "accel": "/usr/libexec/max-ui-accel",
    "strongswan": "/usr/libexec/max-ui-strongswan",
    "amneziawg": "/usr/src/max-ui-amneziawg",
}
...
def push_panel(binary: str, bindir: str):
    ...
    incus("file", "push", binary, f"{VM}{REMOTE}/max-ui", "--mode", "0755", timeout=900)
    ...
    vmsh(f"cd {REMOTE} && ./max-ui --port {PORT} --user admin --pass admin --path / "
    ...
    vmsh("systemctl reset-failed max-ui-test 2>/dev/null; "
         "systemctl stop max-ui-test 2>/dev/null; true", timeout=60)
    rc, out, err = vmsh(f"systemd-run --unit=max-ui-test --working-directory={REMOTE} "
                        f"{REMOTE}/max-ui", timeout=120)
    if rc != 0:
...
def persisted_modules() -> set:
    rc, out, _ = vmsh("cat /etc/modules-load.d/max-ui.conf 2>/dev/null")
...
def check_removal(core: str):
    ...
    # check misses.
    rc, out, _ = vmsh("find /usr/libexec/max-ui /usr/libexec/max-ui-accel "
                      "/usr/libexec/max-ui-strongswan -mindepth 0 2>/dev/null | head -20")
