...
`max-ui --uninstall --yes` reverses everything the panel installs: it stops and
removes the systemd unit `max-ui.service`, kills the VPN daemons
(openvpn/xl2tpd/pptpd/pluto), deletes the nft table `ip vpn`, removes the /etc
configs, the bundle trees under /usr/libexec/max-ui, the fwmark-1/table-100
...
`max-ui.service` unit. This phase asserts the install is present, runs the
...
UNIT_FILE = "/etc/systemd/system/max-ui.service"
LIBEXEC = "/usr/libexec/max-ui"
BIN_DIR = provision.REMOTE_DIR + "/bin"
DB_FILE = provision.REMOTE_DIR + "/max-ui.db"
ETC_CONFIGS = [
    "/etc/max-ui",
    "/etc/sysctl.d/99-max-ui.conf",
    "/etc/modules-load.d/max-ui.conf",
...
def run(incus: Incus, vm: str, panel: Panel, cfg: dict, result, log=None) -> None:
    ...
    port = cfg["panel"]["port"]
    log(":: uninstall — `max-ui --uninstall --yes` tears everything down + assert clean host")
    ...
    #     the nft `ip vpn` table + /etc/max-ui it creates) runs only for
    #     protocol/bulk/backup selections; the /usr/libexec/max-ui bundle is only
    #     extracted once a protocol daemon runs; the max-ui.service unit only
    ...
    if setup_ran:
        checks.append(("test -e /etc/max-ui", "/etc/max-ui"))
        checks.append(("nft list table ip vpn", "nft table ip vpn"))
    if _ran(PHASE_SYSTEMD):
        checks.insert(0, (f"test -f {UNIT_FILE}", "max-ui.service"))
    ...
    rc_act, _, _ = _ex("systemctl is-active --quiet max-ui")
    ...
    # bundle trees under /usr/libexec/max-ui removed.
