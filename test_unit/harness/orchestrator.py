...
def run_job(spec: dict, index: int, cfg: dict,
    ...
    try:
        ...
        #     unit -> installed `max-ui` unit) on the same port. Before uninstall
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="max-ui incus test unit")
    ...
    # default "test_subject/max-ui" means the binary in the test_subject/ folder
    ...
    bar = style.bold_blue("::")
    print(f"\n{style.bold_blue('╭─')} {style.bold_white('max-ui test unit')}")
