#!/usr/bin/env python3
"""Remote scenario runner — drives the LIVE max-ui panel on a remote server plus
...
ssh (unit = max-ui; RADIUS logs at INFO show in journalctl).
"""
...
def journal_since(pattern, secs=50, extra=""):
    _, out, _ = server_exec(
        f'journalctl -u max-ui --no-pager --since "-{secs} seconds" 2>/dev/null '
