...
The max-ui server runs ONE shared strongSwan charon on UDP 500/4500 serving every
...
Inbound. We write it into swanctl's CA dir `/etc/swanctl/x509ca/max-ui-ca-<id>.pem`
...
def _auth_blocks(inbound, acct, remote_id):
    ...
    if mode == "eap-tls":
        ...
        if ca:
            pushes.append((ca, f"/etc/swanctl/x509ca/max-ui-ca-{inbound.inbound_id}.pem", "0644"))
    ...
    pushes = [(ca, f"/etc/swanctl/x509ca/max-ui-ca-{inbound.inbound_id}.pem", "0644")] if ca else []
...
def connect(client: Client, inbound, which: str,
    ...
    client.push(_STRONGSWAN_DROPIN, "/etc/strongswan.d/99-max-ui-client.conf")
