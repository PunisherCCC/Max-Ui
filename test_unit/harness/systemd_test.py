...
`max-ui --systemd` must install + enable-at-boot + start the panel as a systemd
unit named `max-ui` (/etc/systemd/system/max-ui.service) and then exit
...
as the transient unit `max-ui-panel`, which binds :2083; that unit is stopped
first, otherwise the new `max-ui` unit would fight for the busy port and read as
...
def run(incus: Incus, vm: str, panel: Panel, cfg: dict, result, log=None) -> None:
    ...
    log(f":: systemd — `max-ui --systemd` installs + enables + starts unit {SERVICE!r}")
