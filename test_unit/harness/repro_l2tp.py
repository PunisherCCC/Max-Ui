...
if not ok1:
    ...
    print("server pptpd/l2tp:", R.server_exec("pgrep -af 'pptpd|xl2tpd' | head -3; "
          "journalctl -u max-ui --since '-30 sec' --no-pager 2>/dev/null | grep -iE 'pptp|GRE|CTRL' | tail -4")[1])
if ok1:
...
print(R.server_exec('journalctl -u max-ui --since "-35 sec" --no-pager 2>/dev/null '
