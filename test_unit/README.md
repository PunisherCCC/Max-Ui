...
Automated end-to-end test harness for the max-ui panel across a matrix of Linux
...
```bash
# place the prebuilt amd64 binary named `max-ui` (+ its bin/ dir) in test_subject/
...
| key | meaning |
|-----|---------|
| `binary` | path to prebuilt max-ui panel binary (amd64, static, embedded daemons); default `test_subject/max-ui`, pushed with its sibling `bin/` dir |
...
test_subject/              the max-ui binary under test + its bin/ (xray + geo)
  max-ui                   prebuilt panel binary (amd64, static, embedded daemons)
