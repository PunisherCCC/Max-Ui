...
const (
	...
	// /usr/libexec/max-ui prefix but in its own subdir so it never collides with
	// the pppd bundle's sbin/ and lib/.
	LibreswanBundleRoot = "/usr/libexec/max-ui/libreswan"
...
// root (its entries are rooted at usr/libexec/max-ui/libreswan). Idempotent;
