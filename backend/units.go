...
Description=Level 2 Tunnel Protocol Daemon (L2TP) [max-ui bundled]
...
func openvpnTemplateUnit(binDir string) string {
	return fmt.Sprintf(`[Unit]
Description=OpenVPN service for %%I [max-ui bundled]
...
func pptpdUnit(binDir, envLine, pppArg string) string {
	return fmt.Sprintf(`[Unit]
Description=PoPToP PPTP Server [max-ui bundled]
After=network-online.target
Wants=network-online.target
