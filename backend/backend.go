...
// openvpn/libreswan/pppd) directly into the max-ui executable via go:embed and
...
// the bundle works regardless of where max-ui is installed.
const PptpCtrlLink = "/usr/libexec/max-ui/pptpctrl"
...
// file lands flat in bin/ with no sub-folder — resolved next to the max-ui
// executable, so it adapts to any install location (/usr/local/max-ui,
// /usr/lib/max-ui, …). An absolute VPNUI_BIN_FOLDER is honored as-is.
func BinDir() string {
	...
	if err != nil {
		return filepath.Join("/usr/local/max-ui", bin)
	}
