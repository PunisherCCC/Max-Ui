// Package model defines the database models and data structures used by the max-ui panel.
package model
...
// User represents an admin account in the max-ui panel.
//
...
type Inbound struct {
	...
	// Traffic Multiplier: weight a client's usage for accounting only. When enabled,
	// each uploaded and downloaded byte counts TrafficMultiplier times against quota
	// from byte zero. Applies to every protocol and does not affect throughput.
	...
	TrafficMultiplierAfter  int64   `json:"trafficMultiplierAfter" form:"trafficMultiplierAfter" gorm:"default:0"`   // Deprecated; always written as 0
	TrafficMultiplier       float64 `json:"trafficMultiplier" form:"trafficMultiplier" gorm:"default:1"`             // Accounting weight
...
// Setting stores key-value configuration settings for the max-ui panel.
type Setting struct {
	Id    int    `json:"id" form:"id" gorm:"primaryKey;autoIncrement"`
