...
# MmD
#
# max-ui management menu: the `max-ui` command.
#
# Installed to /usr/bin/max-ui by `max-ui-amd64 install-menu`, which deploy.sh runs
...
# it branches on comes from `max-ui-amd64 info --get <field>`, whose field names
...
BIN="${VPNUI_BIN:-/opt/max-ui/max-ui-amd64}"
...
# Read one field of `max-ui-amd64 info --json` by name; prints the raw value.
# Tolerant of failure (empty output) so a caller under `set -e` can branch on it.
info_get() { "$BIN" info --get "$1" 2>/dev/null || true; }

# The panel's systemd unit. NEVER hardcode "max-ui": the name is operator
...
# (setsid ./max-ui-amd64 &) with the unit inactive-but-enabled. systemd then
...
say_unmanaged() {
    ...
    act  "it was started outside systemd (e.g. ${TEAL}setsid ./max-ui-amd64 &${R}), so systemd cannot manage it."
}
...
obtain_letsencrypt_cert() {
    ...
    local unit; unit="$(info_get systemdUnit)"; [[ -n "$unit" ]] || unit="max-ui"
...
# 2) Un-Install. The binary prompts for confirmation and removes /usr/bin/max-ui
...
environment:
  VPNUI_BIN   path to the panel binary (default: /opt/max-ui/max-ui-amd64)
EOF
}

# Acquire root: re-exec through sudo when not already root, so `max-ui` just works
...
require_bin() {
    [[ -x "$BIN" ]] || die "panel binary not found at ${BIN}. Set VPNUI_BIN=/path/to/max-ui-amd64 if it lives elsewhere."
}
