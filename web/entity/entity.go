// Package entity defines data structures and entities used by the web layer of the max-ui panel.
package entity
...
// AllSetting contains all configuration settings for the max-ui panel including web server, Telegram bot, and subscription settings.
type AllSetting struct {
	...
	ProxyCore                   string `json:"proxyCore" form:"proxyCore"`                                     // Active proxy core: xray or sing-box
	SingBoxTemplateConfig       string `json:"singBoxTemplateConfig" form:"singBoxTemplateConfig"`             // Sing-box JSON template
...
func (s *AllSetting) CheckValid() error {
	...
	switch strings.ToLower(strings.TrimSpace(s.ProxyCore)) {
	case "", "xray", "sing-box", "singbox":
	default:
		return common.NewError("proxy core is not valid:", s.ProxyCore)
	}
	if strings.TrimSpace(s.SingBoxTemplateConfig) != "" && !json.Valid([]byte(s.SingBoxTemplateConfig)) {
		return common.NewError("sing-box template config is not valid JSON")
	}
