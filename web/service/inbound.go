// Package service provides business logic services for the max-ui web panel,
...
func (s *InboundService) UpdateInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	...
	oldInbound.TrafficMultiplierAfter = 0
...
func (s *InboundService) addInboundTraffic(tx *gorm.DB, traffics []*xray.Traffic) error {
	...
	for _, traffic := range traffics {
		if traffic.IsInbound && traffic.Tag != "" && !seen[traffic.Tag] {
			seen[traffic.Tag] = true
			tags = append(tags, traffic.Tag)
		}
	}
	inboundsByTag := map[string]*model.Inbound{}
	if len(tags) > 0 {
		var inbounds []*model.Inbound
		cols := append([]string{}, multiplierColumns...)
		cols = append(cols, "tag", "up", "down")
		if err := tx.Model(model.Inbound{}).
			Select(cols).
			Where("tag IN (?)", tags).Find(&inbounds).Error; err != nil {
			logger.Warning("traffic multiplier: cannot load inbound counters, counting inbound traffic raw: ", err)
		} else {
			for _, inbound := range inbounds {
				inboundsByTag[inbound.Tag] = inbound
			}
		}
	}
	...
	for _, traffic := range traffics {
		if traffic.IsInbound {
			billedUp, billedDown := multiplyDelta(inboundsByTag[traffic.Tag], 0, traffic.Up, traffic.Down)
			err = tx.Model(&model.Inbound{}).Where("tag = ?", traffic.Tag).
				Updates(map[string]any{
					"up":       gorm.Expr("up + ?", billedUp),
					"down":     gorm.Expr("down + ?", billedDown),
