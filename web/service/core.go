...
func (s *CoreService) xrayStatus() CoreStatus {
	...
	if s.xrayService.IsXrayCoreRunning() {
...
func (s *CoreService) singBoxStatus() CoreStatus {
	cs := CoreStatus{Name: "sing-box"}
	if s.xrayService.IsSingBoxRunning() {
		cs.State = CoreRunning
		cs.Version = s.xrayService.GetSingBoxVersion()
		return cs
	}
	if err := s.xrayService.GetSingBoxErr(); err != nil {
		cs.State = CoreError
		cs.Detail = err.Error()
		return cs
	}
...
func (s *CoreService) RestartAll() error {
	var errs []string
	for _, name := range []string{s.xrayService.settingService.GetProxyCore(), "l2tp", "pptp", "openvpn", "openconnect", "sstp", "ikev2", "wgc", "awg", "mtproto", "ssh", "radius"} {
...
func (s *CoreService) StopCore(name string) error {
	switch name {
	case "xray":
		return s.xrayService.StopXrayCore()
	case "sing-box":
		return s.xrayService.StopSingBox()
	case "l2tp":
...
func (s *CoreService) runProvisionSteps(emit func(ProvisionStep), cores []string) (rebootModules []string, rebootPkg string) {
	...
	err := os.WriteFile("/etc/modules-load.d/max-ui.conf", []byte(modConf), 0644)
	emit(ProvisionStep{Name: "persist /etc/modules-load.d/max-ui.conf", OK: err == nil, Msg: msgOrOK(err)})
	...
	err = os.WriteFile("/etc/sysctl.d/99-max-ui.conf", []byte(sysctlConf), 0644)
	emit(ProvisionStep{Name: "persist /etc/sysctl.d/99-max-ui.conf", OK: err == nil, Msg: msgOrOK(err)})
