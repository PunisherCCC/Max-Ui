...
func (s *XrayService) IsXrayRunning() bool {
	if s.settingService.GetProxyCore() == "sing-box" {
		return sbp != nil && sbp.IsRunning()
	}
	return p != nil && p.IsRunning()
}

func (s *XrayService) IsXrayCoreRunning() bool {
	return p != nil && p.IsRunning()
}

func (s *XrayService) IsSingBoxRunning() bool {
	return sbp != nil && sbp.IsRunning()
}

func (s *XrayService) GetSingBoxVersion() string {
	if sbp == nil {
		return "Unknown"
	}
	return sbp.GetVersion()
}

func (s *XrayService) GetSingBoxErr() error {
	if sbp == nil {
		return nil
	}
	return sbp.GetErr()
...
func (s *XrayService) RestartXray(isForce bool) error {
	lock.Lock()
	defer lock.Unlock()
	activeCore := s.settingService.GetProxyCore()
	logger.Debug("restart proxy core:", activeCore, "force:", isForce)
	...
	if p != nil && p.IsRunning() {
		if activeCore == "xray" && !isForce && p.GetConfig().Equals(xrayConfig) && !isNeedXrayRestart.Load() {
	...
	if sbp != nil && sbp.IsRunning() {
		if activeCore == "sing-box" && !isForce && bytes.Equal(sbp.GetConfig(), singBoxConfig) && !isNeedXrayRestart.Load() {
...
func (s *XrayService) StopXray() error {
	...
	logger.Debug("Attempting to stop active proxy core...")
	if s.IsXrayRunning() {
		if s.settingService.GetProxyCore() == "sing-box" {
			return sbp.Stop()
		}
	...
	return errors.New(s.settingService.GetProxyCore() + " is not running")
}

func (s *XrayService) StopXrayCore() error {
	lock.Lock()
	defer lock.Unlock()
	isManuallyStopped.Store(true)
	if p != nil && p.IsRunning() {
...
func (s *XrayService) StopSingBox() error {
	lock.Lock()
	defer lock.Unlock()
	isManuallyStopped.Store(true)
	if sbp != nil && sbp.IsRunning() {
