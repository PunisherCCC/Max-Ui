...
func (s *CoreService) runCoreUninstall(selected []string, emit func(ProvisionStep)) []string {
	...
	if len(remaining) == 0 {
		emit(ProvisionStep{Name: "persist /etc/modules-load.d/max-ui.conf", OK: true,
			Msg: removedMsg(removeIfPresent("/etc/modules-load.d/max-ui.conf"))})
	} else {
		...
		err := os.WriteFile("/etc/modules-load.d/max-ui.conf", []byte(strings.Join(dedupe(mods), "\n")+"\n"), 0644)
		emit(ProvisionStep{Name: "persist /etc/modules-load.d/max-ui.conf", OK: err == nil,
			Msg: msgOrOK(err)})
	}
...
// the roots two features share (/usr/libexec/max-ui holds both the pppd bundle
