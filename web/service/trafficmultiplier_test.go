...
// policy builds an enabled accounting multiplier policy. The deprecated `after`
// field is intentionally still accepted here to prove it no longer changes billing.
...
func TestMultiplyDelta(t *testing.T) {
	tests := []struct {
		...
		// The user's example: a 2x multiplier bills 10 MB of actual usage as 20 MB.
		{"doubles from first byte", policy(gb, 2), 0, 0, 10 * mb, 0, 20 * mb},
		{"current counter is ignored", policy(gb, 2), 999 * gb, 5 * mb, 5 * mb, 10 * mb, 10 * mb},
		{"deprecated threshold is ignored", policy(gb, 2), gb - 1, 0, 1, 0, 2},
		{"same multiplier applies to both directions", policy(gb, 2), 0, 5 * mb, 5 * mb, 10 * mb, 10 * mb},
		{"three times weights everything", policy(0, 3), 0, 10 * mb, 20 * mb, 30 * mb, 60 * mb},
...
// seedMultiplierDB builds a real SQLite with one policy inbound (2x accounting)
// and one plain inbound, each with a client, and returns their emails.
func seedMultiplierDB(t *testing.T) (multipliedEmail, plainEmail string) {
	...
	multiplied := &model.Inbound{
		...
		TrafficMultiplierEnable: true, TrafficMultiplierAfter: 0, TrafficMultiplier: 2,
	}
	plain := &model.Inbound{
...
func TestAddInboundTrafficAppliesMultiplier(t *testing.T) {
	if err := database.InitDB(filepath.Join(t.TempDir(), "test.db")); err != nil {
		t.Fatalf("InitDB: %v", err)
	}
	db := database.GetDB()

	weighted := &model.Inbound{
		UserId: 1, Tag: "inbound-30001", Port: 30001, Protocol: model.VMESS, Enable: true,
		TrafficMultiplierEnable: true, TrafficMultiplierAfter: 0, TrafficMultiplier: 2,
	}
	plain := &model.Inbound{
		UserId: 1, Tag: "inbound-30002", Port: 30002, Protocol: model.VMESS, Enable: true,
		TrafficMultiplier: 1,
	}
	for _, ib := range []*model.Inbound{weighted, plain} {
		if err := db.Create(ib).Error; err != nil {
			t.Fatalf("create inbound %s: %v", ib.Tag, err)
		}
	}
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		return s.addInboundTraffic(tx, []*xray.Traffic{
			{IsInbound: true, Tag: weighted.Tag, Up: 100 * mb, Down: 50 * mb},
			{IsInbound: true, Tag: plain.Tag, Up: 100 * mb, Down: 50 * mb},
		})
	})
	if err != nil {
	...
	var gotWeighted, gotPlain model.Inbound
	if err := db.Where("id = ?", weighted.Id).First(&gotWeighted).Error; err != nil {
		t.Fatalf("read weighted inbound: %v", err)
	}
	if err := db.Where("id = ?", plain.Id).First(&gotPlain).Error; err != nil {
		t.Fatalf("read plain inbound: %v", err)
	}
	if got := gotWeighted.Up + gotWeighted.Down; got != 300*mb {
		t.Errorf("weighted inbound accounting = %d; want %d", got, 300*mb)
	}
	if gotWeighted.AllTime != 150*mb {
		t.Errorf("weighted inbound all_time = %d; want raw %d", gotWeighted.AllTime, 150*mb)
	}
	if got := gotPlain.Up + gotPlain.Down; got != 150*mb {
		t.Errorf("plain inbound accounting = %d; want %d", got, 150*mb)
	}
}
...
func TestFoldClientTrafficAppliesMultiplier(t *testing.T) {
	...
	if got := storedUpDown(t, weighted); got != 400*mb {
		t.Errorf("weighted from first byte: stored %d; want %d", got, 400*mb)
	}

	foldClientTraffic(weighted, 0, 1024*mb)
	if got, want := storedUpDown(t, weighted), int64(400*mb+2048*mb); got != want {
		t.Errorf("second weighted fold: stored %d; want %d", got, want)
	}

	foldClientTraffic(weighted, 0, 10*mb)
	if got, want := storedUpDown(t, weighted), int64(400*mb+2048*mb+20*mb); got != want {
		t.Errorf("third weighted fold: stored %d; want %d", got, want)
	}
...
// The columns must survive AutoMigrate and a save/load round-trip, and existing
// rows must default to a 1x multiplier rather than 0x (which would zero out
// everyone's accounting on upgrade).
