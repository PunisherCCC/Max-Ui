...
// process (`max-ui-amd64 ctl <cmd>`) drive the LIVE panel's Xray and daemons.
//
...
// documents them. Exported so `max-ui-amd64 ctl` prints the set without keeping a
...
// ControlSocketPath returns the control socket's path: max-ui.sock next to the
// panel binary.
//
...
func StartControlSocket() error {
	...
	if conn, derr := net.DialTimeout("unix", path, controlDialTimeout); derr == nil {
		_ = conn.Close()
		return fmt.Errorf("another max-ui panel is already serving %s", path)
	}
...
func handleControlCommand(cmd string) ControlResponse {
	...
	switch cmd {
	case "ping":
		return ControlResponse{OK: true, Cmd: cmd, Msg: fmt.Sprintf("max-ui %s panel is running (pid %d)", config.GetVersion(), os.Getpid())}
