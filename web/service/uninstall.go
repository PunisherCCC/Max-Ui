...
// MenuScriptPath is where the `max-ui` management menu is installed, by
// `max-ui-amd64 install-menu` (which deploy.sh runs on every install and update).
...
const MenuScriptPath = "/usr/bin/max-ui"
...
func Uninstall(opts UninstallOptions) *UninstallReport {
	...
	// 1. The panel's own systemd unit (default "max-ui"). disable --now stops it
	...
	// 1b. The `max-ui` management menu. Unlinking it while it is the very script
	...
	for _, p := range []string{
		"/etc/max-ui", // nft config dir (vpn.nft)
	...
	removePath(r, config.GetLogFolder()) // /var/log/max-ui
	...
	removePath(r, backend.PppdBundleRoot) // /usr/libexec/max-ui (incl. libreswan/, pptpctrl)
	...
	//     /etc/max-ui-ikev2, /etc/strongswan.conf, /var/run/{ocserv,charon.vici}
