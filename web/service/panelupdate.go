...
func (s *ServerService) CheckPanelUpdate() (*PanelUpdateInfo, error) {
	...
	req.Header.Set("User-Agent", "max-ui") // GitHub API rejects requests without a UA
...
func (s *ServerService) UpdatePanel() error {
	...
	// back manually (mv max-ui.bak max-ui): once renamed, the old inode is gone.
...
func DownloadPanelBinary(ctx context.Context, dst, url string) error {
	...
	req.Header.Set("User-Agent", "max-ui")
	resp, err := client.Do(req)
	if err != nil {
		return err
	}
...
func backupPanelDB() {
	...
	base := fmt.Sprintf("max-ui_%s.db", config.GetVersion())
