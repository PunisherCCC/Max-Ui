...
// distroStatus reports whether the running host distro is on max-ui's tested list,
...
// checkUpdate reports whether a newer max-ui panel release is available. The
...
func (a *ServerController) getDb(c *gin.Context) {
	...
	filename := fmt.Sprintf("max-ui_%s.db", time.Now().Format("20060102-150405"))
...
// importForeignDB imports a stock 3x-ui (or max-ui) backup over the current
