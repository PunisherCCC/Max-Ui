// Package main is the entry point for the max-ui web panel application.
...
// runWebServer initializes and starts the web server for the max-ui panel.
...
func requireRoot() {
	...
	const m = "max-ui must be run as root. It binds privileged ports, writes systemd units, and manages nftables, routing and the VPN daemons.\n       Try: sudo max-ui"
...
// ansiVpnUI renders "[MAX-UI]" in the panel logo's colours — teal brackets,
...
// distro is not on max-ui's tested list (service.DistroSupported). Colorful when
...
func warnUnsupportedDistro() {
	...
	logger.Warningf("unsupported distro: %s (%s) — not officially supported by max-ui, expect errors",
		pretty, reason)

	tested := service.SupportedDistroSummary()
	if os.Getenv("NO_COLOR") != "" || !stdoutIsTTY() {
		fmt.Fprintf(os.Stderr,
			"\nWARNING: %s is NOT officially supported by max-ui. It may run, but expect errors.\n"+
				"Tested distros: %s.\n\n", pretty, tested)
		return
	}
	const (
	...
	fmt.Fprintf(os.Stderr, "%s%s%s is not officially supported by max-ui — %sexpect errors%s.\n",
...
func runWebServer() {
	...
	// Ensure the `max-ui` management menu is installed, so the command works on a
	// hand-deployed box that never ran deploy.sh's `install-menu`. Idempotent and
	// best-effort; see ensureMenuInstalled.
	ensureMenuInstalled()

	// The root-only control socket the `max-ui-amd64 ctl` CLI (and the max-ui menu)
	...
	if err := service.StartControlSocket(); err != nil {
		logger.Warning("control socket unavailable (the max-ui menu's Xray/cores items will not work):", err)
	}
...
// Invoked by `max-ui --systemd`. Must run as root (it writes /etc/systemd/system).
func installSystemd() {
...
// `max-ui --uninstall`; `--yes`/`--force` skips the confirmation prompt. Must run
...
func runUninstall(assumeYes bool) {
	...
	if !assumeYes {
		fmt.Println("This will REMOVE max-ui and everything it installed on this host:")
		...
		fmt.Println("  • /etc configs, /usr/libexec/max-ui bundles, logs, bin/, the database")
		fmt.Println("  • the max-ui binary itself")
	...
	fmt.Println("Uninstalling max-ui...")
	...
	if exePath != "" {
		...
		// their own files in /opt/max-ui does not lose them, while the usual case
...
// can log in. Invoked by `max-ui --random` (composable with --systemd, which is
...
// base path to explicit values from `max-ui --user/--pass/--port/--path`. It uses
...
// `max-ui info`. The JSON field names are a CONTRACT with the max-ui menu script:
...
func collectPanelInfo(resolvePublicIP bool) panelInfo {
	...
	// Never hardcode "max-ui": the unit name is operator-configurable (settings key
...
// runInfo implements `max-ui info [--json|--get <field>]`: the panel's login,
...
// runCtl implements `max-ui ctl <cmd> [--json]`: hand one command to the RUNNING
...
func runCtl(args []string) {
	...
	if cmd == "" {
		fmt.Fprintf(os.Stderr, "usage: max-ui ctl <command> [--json]\ncommands: %s\n",
	...
	conn, err := ctlDial()
	if err != nil {
		fmt.Fprintf(os.Stderr, "the max-ui panel is not running: no answer on %s (%v)\n",
...
// runUpdate implements `max-ui update`: install the latest published release over
// this binary.
//
...
func runUpdate() {
	...
	// Refresh /usr/bin/max-ui from the NEW binary, not from this (outgoing) one: the
	...
	// (setsid ./max-ui-amd64 &) the OLD binary keeps serving until someone kills it.
...
// and single-slot (max-ui_<version>.db), so a second update from the same version
...
func backupPanelDBForUpdate(fromVersion string) (string, error) {
	...
	dst := filepath.Join(dir, fmt.Sprintf("max-ui_%s_%s.db", fromVersion, time.Now().Format("20060102-150405")))
...
// menuScript is the `max-ui` management menu, shipped INSIDE the binary it drives.
//
...
// max-ui.sh used to acquire it with `curl https://get.acme.sh | sh`, which fails on
...
// installAcmeScript implements `max-ui install-acme <path>`: write the embedded
...
func installAcmeScript(args []string) {
	if len(args) == 0 || args[0] == "" {
		fmt.Fprintln(os.Stderr, "usage: max-ui install-acme <path>")
		os.Exit(1)
	}
...
// installMenuScript implements `max-ui install-menu [path]`: write the embedded
// menu to /usr/bin/max-ui (0755). deploy.sh runs it on both fresh install and
...
func installMenuScript(args []string) {
	...
	// this via `max-ui-amd64 update`. bash reads a script lazily by offset from an
	...
	fmt.Printf("Installed the max-ui management menu -> %s (run: %s)\n", dest, filepath.Base(dest))
}

// ensureMenuInstalled writes the `max-ui` management menu to MenuScriptPath on panel
// startup when it is missing or out of date, so the `max-ui` command exists no matter
// how the panel was deployed. deploy.sh runs `install-menu` explicitly, but a manual
// launch (scp the binary, `setsid ./max-ui-amd64 &`) never does, so the command was
...
func ensureMenuInstalled() {
	...
	if err := backend.WriteFileAtomic(dest, menuScript, 0o755); err != nil {
		logger.Warning("could not install the max-ui management menu at", dest, ":", err)
		return
	}
	logger.Info("installed the max-ui management menu at", dest)
}
...
// `max-ui cert -selfsign` (used by deploy.sh's fresh-install HTTPS option). The
...
// migrateDb performs database migration operations for the max-ui panel.
func migrateDb() {
...
// importDb imports a stock 3x-ui (or max-ui) backup database over the current one,
...
// 3x-ui backup; it is also available standalone. Usage: max-ui import --from <path>
func importDb() {
	importCmd := flag.NewFlagSet("import", flag.ExitOnError)
	var from string
	importCmd.StringVar(&from, "from", "", "path to the 3x-ui/max-ui backup .db to import")
	_ = importCmd.Parse(os.Args[2:])
	if from == "" && importCmd.NArg() > 0 {
		from = importCmd.Arg(0) // also accept `max-ui import <path>`
	}
	if from == "" {
		fmt.Fprintln(os.Stderr, "usage: max-ui import --from <path-to-backup.db>")
		os.Exit(1)
	}
...
// Usage: max-ui openvpn-auth {inbound_id} {credentials_file}
// The credentials file has username on line 1, password on line 2.
func openvpnAuth() {
	if len(os.Args) < 4 {
		fmt.Fprintln(os.Stderr, "usage: max-ui openvpn-auth <inbound_id> <cred_file>")
		os.Exit(1)
	}
...
// Usage: max-ui openvpn-evict <id> <proto> <ip> [real-address]
func openvpnEvict() {
	if len(os.Args) < 5 {
		return
	}
...
// Usage: max-ui openvpn-connect {inbound_id}
// Reads common_name and ifconfig_pool_remote_ip from environment.
func openvpnConnect() {
	if len(os.Args) < 3 {
		fmt.Fprintln(os.Stderr, "usage: max-ui openvpn-connect <inbound_id>")
		os.Exit(1)
	}
...
// Usage: max-ui openvpn-disconnect {inbound_id}
// Reads common_name and ifconfig_pool_remote_ip from environment.
func openvpnDisconnect() {
	if len(os.Args) < 3 {
		fmt.Fprintln(os.Stderr, "usage: max-ui openvpn-disconnect <inbound_id>")
		os.Exit(1)
	}
...
// main is the entry point of the max-ui application.
...
func main() {
	...
	//   max-ui --random --systemd
	//   max-ui --user admin --pass s3cret --port 8443 --path panel --systemd
	...
	flag.Usage = func() {
		...
		fmt.Println("    migrate        migrate form other/old max-ui")
		fmt.Println("    import         import a 3x-ui/max-ui backup DB over the current one")
		...
		fmt.Println("    install-menu   install the 'max-ui' management menu to " + service.MenuScriptPath)
